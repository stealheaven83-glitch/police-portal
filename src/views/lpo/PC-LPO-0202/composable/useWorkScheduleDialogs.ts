...
export const workKindOptions: SelectOption[] = [
  { label: "순찰차", value: "순찰차" },
  { label: "중점사항", value: "중점사항" },
  { label: "상황", value: "상황" },
  { label: "도보", value: "도보" },
  { label: "오토바이/자전거", value: "오토바이/자전거" },
  { label: "병합근무(순찰상황)", value: "병합근무(순찰상황)" },
  { label: "병합근무(휴게)", value: "병합근무(휴게)" },
  { label: "병합근무(대기)", value: "병합근무(대기)" },
  { label: "기타", value: "기타" },
  { label: "병원(기타)", value: "병원(기타)" },
  { label: "지역안전순찰(전담)", value: "지역안전순찰(전담)" },
  { label: "지역안전순찰(병합)", value: "지역안전순찰(병합)" },
  { label: "상시교육(개별)", value: "상시교육(개별)" },
  { label: "상시교육(전체)", value: "상시교육(전체)" },
  { label: "거점", value: "거점" },
  { label: "협업지원", value: "협업지원" },
];
...
export const hourOptions: SelectOption[] = Array.from(
  { length: 24 },
  (_, h) => {
...
export function useWorkScheduleDialogs() {
  ...
  function addWorkKindRow() {
    const nextId =
      workKindRows.value.length ?
        Math.max(...workKindRows.value.map((r) => r.id)) + 1
      : 1;
    const nextOrder =
      workKindRows.value.length ?
        Math.max(...workKindRows.value.map((r) => r.order)) + 1
      : 1;
    workKindRows.value = [
      ...workKindRows.value,
      { id: nextId, work: "", kind: "상황", order: nextOrder, used: false },
    ];
  }
  ...
  function addWorkTimeRows() {
    let nextId =
      workTimeRows.value.length ?
        Math.max(...workTimeRows.value.map((r) => r.id)) + 1
      : 1;
  ...
  function addPatrolPointRow() {
    const nextId =
      patrolPointRows.value.length ?
        Math.max(...patrolPointRows.value.map((r) => r.id)) + 1
      : 1;
    const nextOrder =
      patrolPointRows.value.length ?
        Math.max(...patrolPointRows.value.map((r) => r.order)) + 1
      : 1;
