<template>
  <GenericDialog2 v-model:open="open" title="범죄예방진단결과 (CPO확인용)" :size="1800" show-close-button :show-footer="false">
    ...
    -->
    <LayoutSplite :count="2" :widths="[42, 58]" class="lp-split-popup">
      <template #layout-1>
        <LayoutPanel title="범죄예방진단통보 현황">
        <!--
        ...
        <LayoutPanel title="범죄예방진단통보 상세">
          ...
          <InfoTable :columns="2" popup size="120">
            ...
            <InfoField label="부서" full>
              {{ diagnosis?.dept ?? '부산청 부산중부서 송도지구대' }}
            ...
            <InfoField for="cpo-diagnosis-date" label="진단일자" class="lp-date-fill">
              <DatePicker
            ...
            <!-- 시안: 기본/상세주소 두 줄 옆에 이력보기(76×32 = size xs + padding 12)가 세로 가운데로 붙는다.
                 lp-flex-fill 이 주소 묶음에 남는 폭을 줘서 버튼이 오른쪽 끝으로 간다 -->
            <InfoField label="주소" :row-span="2">
              <span class="lp-cell-lines lp-flex-fill">
                <span class="readonly-text">{{ baseAddressText }}</span>
                <span class="readonly-text">{{ detailAddressText }}</span>
              </span>
              <Button type="button" variant="tertiary" size="xs" padding="12" @click="emit('open-history')">이력보기</Button>
            </InfoField>
          ...
          <InfoTable :columns="2" popup size="120">
            <InfoField label="직원수"><InputField2 v-model.number="form.employeeCount" type="number" :clearable="false" size="sm" min="0" aria-label="직원수" class="!space-y-0 flex-1" input-class="w-full" /></InfoField>
            <InfoField label="업소층수"><InputField2 v-model.number="form.floorCount" type="number" :clearable="false" size="sm" min="0" aria-label="업소층수" class="!space-y-0 flex-1" input-class="w-full" /></InfoField>

            <InfoField label="입주년도"><InputField2 v-model.number="form.moveInYear" type="number" :clearable="false" size="sm" min="0" aria-label="입주년도" class="!space-y-0 flex-1" input-class="w-full" /></InfoField>
            ...
            <InfoField label="피해 횟수"><InputField2 v-model.number="form.damageCount" type="number" :clearable="false" size="sm" min="0" aria-label="피해 횟수" class="!space-y-0 flex-1" input-class="w-full" /></InfoField>
          </InfoTable>
