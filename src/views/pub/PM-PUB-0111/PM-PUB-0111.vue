<template>
  ...
  <div class="list-actions">
    ...
    <Button type="button" variant="tertiary" size="sm" @click="onUpload">업로드</Button>
    <Button type="button" variant="primary" size="sm" @click="onSave">저장</Button>
  </div>

  <div class="lp-page-scroll">
    <section class="lp-section" aria-labelledby="ref-crime-heading">
      <h2 id="ref-crime-heading" class="lp-heading-md lp-section-title">범죄 통계</h2>
      <InfoTable :columns="2" size="130">
        <!-- 시안은 '범죄' 한 칸이 하위 3줄을, '강력범죄' 한 칸이 하위 4줄을 묶는다 -->
        <InfoField label="범죄" class="lp-info-nested">
          <InfoTable :columns="1" size="110">
            <InfoField label="철도범죄">
              <Stepper v-model="crime.railway" :min="0" label="철도범죄" />
            </InfoField>
            <InfoField label="폭력범죄">
              <Stepper v-model="crime.violence" :min="0" label="폭력범죄" />
            </InfoField>
            <InfoField label="지능범죄">
              <Stepper v-model="crime.intelligence" :min="0" label="지능범죄" />
            </InfoField>
          </InfoTable>
        </InfoField>

        <InfoField label="강력범죄" class="lp-info-nested">
          <InfoTable :columns="1" size="150">
            <InfoField label="강력(살인)">
              <Stepper v-model="crime.murder" :min="0" label="강력(살인)" />
            </InfoField>
            <InfoField label="강력(강도)">
              <Stepper v-model="crime.robbery" :min="0" label="강력(강도)" />
            </InfoField>
            <InfoField label="강력(강간, 강제추행)">
              <Stepper v-model="crime.rape" :min="0" label="강력(강간, 강제추행)" />
            </InfoField>
            <InfoField label="강력(방화)">
              <Stepper v-model="crime.arson" :min="0" label="강력(방화)" />
            </InfoField>
          </InfoTable>
        </InfoField>
    ...
    <section class="lp-section" aria-labelledby="ref-report-heading">
      ...
      <InfoTable :columns="2" size="130">
        <InfoField label="Code 1">
          <Stepper v-model="report.code1" :min="0" label="Code 1" />
        </InfoField>
        <InfoField label="Code 2">
          <Stepper v-model="report.code2" :min="0" label="Code 2" />
        </InfoField>
    ...
    <section class="lp-section" aria-labelledby="ref-population-heading">
      ...
      <InfoTable :columns="2" size="130">
        <InfoField label="지피면적">
          <Stepper v-model="population.landArea" :min="0" label="지피면적" />
        </InfoField>
        <InfoField label="면적">
          <Stepper v-model="population.area" :min="0" label="면적" />
        </InfoField>

        <InfoField label="인구밀도">
          <Stepper v-model="population.density" :min="0" label="인구밀도" />
        </InfoField>
        <InfoField label="전년인구">
          <Stepper v-model="population.prevPopulation" :min="0" label="전년인구" />
        </InfoField>

        <InfoField label="인구이동">
          <Stepper v-model="population.migration" :min="0" label="인구이동" />
        </InfoField>
        <InfoField label="기초생활수급자수">
          <Stepper v-model="population.basicLivingCount" :min="0" label="기초생활수급자수" />
        </InfoField>

        <InfoField label="기초생활수급자">
          <Stepper v-model="population.basicLiving" :min="0" label="기초생활수급자" />
        </InfoField>
        <InfoField label="전체세대">
          <Stepper v-model="population.households" :min="0" label="전체세대" />
        </InfoField>

        <InfoField label="1인가구 비율">
          <Stepper v-model="population.singleHouseholdRate" :min="0" label="1인가구 비율" />
        </InfoField>
        <InfoField label="1인세대">
          <Stepper v-model="population.singleHouseholds" :min="0" label="1인세대" />
        </InfoField>

        <InfoField label="외국인 비율">
          <Stepper v-model="population.foreignerRate" :min="0" label="외국인 비율" />
        </InfoField>
        <InfoField label="등록 외국인">
          <Stepper v-model="population.registeredForeigners" :min="0" label="등록 외국인" />
        </InfoField>

        <InfoField label="관리대상자수">
          <Stepper v-model="population.managedTargets" :min="0" label="관리대상자수" />
        </InfoField>
        <InfoField label="우범자수">
          <Stepper v-model="population.exOffenders" :min="0" label="우범자수" />
        </InfoField>

        <!-- 풍속업소는 아래 업종별 합계라 입력이 아니다(시안에서도 빈 칸이다) -->
        <InfoField label="풍속업소">
          <span class="readonly-text">{{ entertainmentTotal }}</span>
        </InfoField>
        <InfoField label="신상정보등록대상자수">
          <Stepper v-model="population.registeredIdentityTargets" :min="0" label="신상정보등록대상자수" />
        </InfoField>

        <InfoField label="인구">
          <Stepper v-model="population.population" :min="0" label="인구" />
        </InfoField>
      </InfoTable>

      <InfoTable :columns="2" size="130" class="form-rest lp-table-gap">
        <InfoField label="유흥주점">
          <Stepper v-model="entertainment.bar" :min="0" label="유흥주점" />
        </InfoField>
        <InfoField label="노래연습장">
          <Stepper v-model="entertainment.karaokeRoom" :min="0" label="노래연습장" />
        </InfoField>

        <InfoField label="단란주점">
          <Stepper v-model="entertainment.pub" :min="0" label="단란주점" />
        </InfoField>
        <InfoField label="게임제공업">
          <Stepper v-model="entertainment.gameArcade" :min="0" label="게임제공업" />
        </InfoField>

        <InfoField label="숙박업">
          <Stepper v-model="entertainment.lodging" :min="0" label="숙박업" />
        </InfoField>
        <InfoField label="무도학원">
          <Stepper v-model="entertainment.danceAcademy" :min="0" label="무도학원" />
        </InfoField>

        <InfoField label="이용업">
          <Stepper v-model="entertainment.barbershop" :min="0" label="이용업" />
        </InfoField>
        <InfoField label="무도장">
          <Stepper v-model="entertainment.danceHall" :min="0" label="무도장" />
        </InfoField>

        <InfoField label="비디오감상실">
          <Stepper v-model="entertainment.videoRoom" :min="0" label="비디오감상실" />
        </InfoField>
...
async function onUpload() {
  await dialog.alert({ title: '업로드는 연동 후 제공됩니다.', btnCancel: '확인' })
}
