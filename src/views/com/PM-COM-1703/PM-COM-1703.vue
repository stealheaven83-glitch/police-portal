...
import { useTipEdit } from './composable/PM-COM-1703'
...
const navItems = [
  ...
  { label: '게시판' },
  { label: '교육자료 나눔터' },
  { label: '현장대응팁', path: '/views/com/PM-COM-1701' },
]
 ...
 * 제목이 없는 경우는 버튼 자체가 비활성(등록 시안 PM-COM-1704 와 같은 규칙)이라 여기까지 오지 않는다.
...
async function onSave() {
  ...
  if (!confirmed) return
  router.push({ name: 'PM-COM-1701' })
}

useBottomTabSetup({
  value: 'PM-COM-1703',
  label: '현장대응팁 수정',
  path: '/views/com/PM-COM-1703',
  componentName: 'PmCom1703',
  closable: true,
})
</script>
