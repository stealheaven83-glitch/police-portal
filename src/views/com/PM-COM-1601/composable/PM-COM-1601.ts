...
/**
 * 법령 · 지침 · 매뉴얼 목록(PM-COM-1601) 한 줄.
 *
 * 칸 구성: 부서 X / 공지 고정 O / 추천수 X / 공개상태 X / 카테고리 X.
 * 상세·수정·등록(PM-COM-1602~0712)은 `useBoardStore('law')` 의 상태를 쓰고,
...
function createMockRows(): LawRow[] {
...
export function useLawList() {
  ...
  const allRows = ref<LawRow[]>(createMockRows())
