...
/**
 * 법령 · 지침 · 매뉴얼 수정(PM-COM-1603) 화면 상태.
 * 폼 값은 도메인 스토어(board.ts)의 editForm 을 그대로 쓴다 — 상세(PM-COM-1602)의 '수정' 버튼이
...
export function useLawEdit() {
  const { current, editForm, loadEditForm } = useBoardStore('law')
  ...
  /** 등록 시안(PM-COM-1604)처럼 제목이 비어 있으면 저장 버튼을 비활성으로 둔다 */
