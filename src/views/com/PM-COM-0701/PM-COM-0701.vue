...
import { useTrainingCaseList, type TrainingCaseRow } from './composable/PM-COM-0701'
import {
  BOARD_SPECS,
  boardMenu,
  useBoardStore,
...
function titleFormatter(cell: any) {
  const row = cell.getData() as TrainingCaseRow
...
/** 추천수는 이 게시판에 없다(BOARD_SPECS.training.hasRecommend = false) */
...
async function onDeleteSelected() {
  ...
  const ids = (gridRef.value?.getSelectedData() ?? []).map((row: TrainingCaseRow) => row.id)
  removeRows(ids)
