...
/**
 * 경찰청 주요 정책 목록(PM-COM-0901) 한 줄.
 *
 * 칸 구성: 부서 X / 공지 고정 O / 추천수 X / 공개상태 X / 카테고리 X.
 * 상세·수정·등록(PM-COM-0902~0712)은 `useBoardStore('policePlan')` 의 상태를 쓰고,
...
function createMockRows(): PolicyRow[] {
...
export function usePolicyList() {
  ...
  const allRows = ref<PolicyRow[]>(createMockRows())
