...
import { usePolicyList, type PolicyRow } from './composable/PM-COM-0901'
import {
  boardMenu,
  useBoardStore,
  authorFilterOptions,
...
const { selectPost } = useBoardStore('policePlan')
...
function titleFormatter(cell: any) {
  const row = cell.getData() as PolicyRow
...
async function onDeleteSelected() {
  ...
  const ids = (gridRef.value?.getSelectedData() ?? []).map((row: PolicyRow) => row.id)
  removeRows(ids)
...
function onCreate() {
  router.push({ name: 'PM-COM-0904' })
}

/** 제목을 누르면 상세로 이동 */
function onRowClick(_event: unknown, row: { getData: () => { id?: number } }) {
  selectPost(row.getData().id ?? 1)
  router.push({ name: 'PM-COM-0902' })
}

useBottomTabSetup({
  value: 'PM-COM-0901',
  label: '경찰청 주요 정책',
  path: '/views/com/PM-COM-0901',
  componentName: 'PmCom0901',
  closable: true,
})
</script>
