...
import { usePolicyEdit } from './composable/PM-COM-0903'
...
const navItems = [
  ...
  { label: '게시판' },
  { label: '현장공감 TalkTalk' },
  { label: '경찰청 주요 정책', path: '/views/com/PM-COM-0901' },
]

const { form, writer, writtenAt, fileCount, canSave, addFiles, removeFile } = usePolicyEdit()
 ...
 * 제목이 없는 경우는 버튼 자체가 비활성(등록 시안 PM-COM-0904 와 같은 규칙)이라 여기까지 오지 않는다.
...
async function onSave() {
  ...
  if (!confirmed) return
  router.push({ name: 'PM-COM-0901' })
}

useBottomTabSetup({
  value: 'PM-COM-0903',
  label: '경찰청 주요 정책 수정',
  path: '/views/com/PM-COM-0903',
  componentName: 'PmCom0903',
  closable: true,
})
</script>
