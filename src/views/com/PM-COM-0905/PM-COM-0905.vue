...
import { useProposalList, type ProposalRow } from './composable/PM-COM-0905'
import {
  boardMenu,
  useBoardStore,
  authorFilterOptions,
...
function onCreate() {
  router.push({ name: 'PM-COM-0908' })
}

/** 제목을 누르면 상세로 이동 */
function onRowClick(_event: unknown, row: { getData: () => { id?: number } }) {
  selectPost(row.getData().id ?? 1)
  router.push({ name: 'PM-COM-0906' })
}

useBottomTabSetup({
  value: 'PM-COM-0905',
  label: '정책 제안 및 건의사항',
  path: '/views/com/PM-COM-0905',
  componentName: 'PmCom0905',
  closable: true,
})
</script>
