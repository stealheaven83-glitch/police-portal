...
/**
 * 정책 제안 및 건의사항 수정(PM-COM-2103) 화면 상태.
 * 폼 값은 도메인 스토어(board.ts)의 editForm 을 그대로 쓴다 — 상세(PM-COM-2102)의 '수정' 버튼이
...
export function useProposalEdit() {
  const { current, editForm, loadEditForm } = useBoardStore('proposal')
  ...
  /** 등록 시안(PM-COM-2104)처럼 제목이 비어 있으면 저장 버튼을 비활성으로 둔다 */
  const canSave = computed(() => editForm.title.trim().length > 0)

  /** 비밀글 체크 = 공개 여부의 반대. 게시글 데이터는 open(공개) 하나로 들고 있다(board.ts) */
