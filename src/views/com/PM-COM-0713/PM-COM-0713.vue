...
import { useTipList, type TipRow } from './composable/PM-COM-0713'
import {
  boardMenu,
  useBoardStore,
  authorFilterOptions,
...
defineOptions({ name: 'PmCom0713' })

// LNB: 게시판 > 교육자료 나눔터 > 현장대응팁
// (presets.ts 에 게시판 메뉴가 아직 없어 도메인 composable 의 구성을 쓴다 — docs/create.md §3)
useSideMenuSetup({ ...boardMenu, openIndex: 4, activeChild: '현장대응팁' })
...
function titleFormatter(cell: any) {
  const row = cell.getData() as TipRow
...
async function onDeleteSelected() {
  ...
  const ids = (gridRef.value?.getSelectedData() ?? []).map((row: TipRow) => row.id)
  removeRows(ids)
...
function onCreate() {
  router.push({ name: 'PM-COM-0716' })
}

/** 제목을 누르면 상세로 이동 */
function onRowClick(_event: unknown, row: { getData: () => { id?: number } }) {
  selectPost(row.getData().id ?? 1)
  router.push({ name: 'PM-COM-0714' })
}

useBottomTabSetup({
  value: 'PM-COM-0713',
  label: '현장대응팁',
  path: '/views/com/PM-COM-0713',
  componentName: 'PmCom0713',
  closable: true,
})
</script>
