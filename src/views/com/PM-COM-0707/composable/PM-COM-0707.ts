...
/**
 * 상시학습자료 수정(PM-COM-0707) 화면 상태.
 * 폼 값은 도메인 스토어(board.ts)의 editForm 을 그대로 쓴다 — 상세(PM-COM-0706)의 '수정' 버튼이
...
export function useStudyEdit() {
  const { current, editForm, loadEditForm } = useBoardStore('study')
  ...
  /** 등록 시안(PM-COM-0708)처럼 제목이 비어 있으면 저장 버튼을 비활성으로 둔다 */
