<template>
  ...
  <div class="lp-notice-scroll">
    <article class="lp-notice-detail">
      <!-- 시안: 공지 · 공개상태 배지가 한 줄. 부서·카테고리 칸은 없다 -->
      <p class="lp-notice-badges">
        <Badge v-if="notice.pinned" color="primary" variant="solid" size="lg" shape="sm">공지</Badge>
        <Badge color="tertiary" variant="outline" size="lg" shape="sm">{{ notice.open ? '공개' : '비공개' }}</Badge>
      </p>
...
useBottomTabSetup({
  ...
  path: '/views/com/PM-COM-0906',
  componentName: 'PmCom0906',
  closable: true,
})
</script>
