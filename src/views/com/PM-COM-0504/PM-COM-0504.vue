<template>
  ...
  <div class="lp-page-scroll">
    <!-- Figma 11220:93469 "list": 블록 사이 20 균일(.lp-narrow-form gap) / 라벨→입력 8(.lp-field gap) -->
    <div class="lp-narrow-form">
      ...
      <!-- Figma: 등록 화면은 내용 칸 아래 글자수 카운터(0/100)가 있다 — 수정 화면(0503)과 다름 -->
      <TextareaField
        id="case-content"
...
import { usePolicyCaseCreate } from './composable/PM-COM-0504'
...
const { form, writer, writtenAt, fileCount, canSave, addFiles, removeFile } = usePolicyCaseCreate()
...
/** 등록은 목록의 '등록' 버튼에서 들어오므로 취소하면 목록으로 돌아간다 */
function onCancel() {
...
useBottomTabSetup({
  value: 'PM-COM-0504',
  label: '우수사례 등록',
  path: '/views/com/PM-COM-0504',
  componentName: 'PmCom0504',
  closable: true,
})
</script>
