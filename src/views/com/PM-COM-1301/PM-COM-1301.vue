...
import { useBoard1301List } from './composable/PM-COM-1301'
import {
  boardMenu,
  BOARD_SPECS,
  useBoardStore,
...
// LNB: 게시판 > 우수사례 > 현장조치 (presets.ts 에 게시판 메뉴가 아직 없어 도메인 composable 의 구성을 쓴다)
...
const { selectPost } = useBoardStore('fieldCase')
...
const columns: TabulatorGridColumn[] = [
  ...
  { title: '주차', field: 'week', width: 100, hozAlign: 'center' },
...
function onRegister() {
  router.push({ name: 'PM-COM-1304' })
}

/** 행을 누르면 상세로 간다 */
function onRowClick(_event: unknown, row: { getData: () => { id?: number } }) {
  selectPost(row.getData().id ?? 1)
  router.push({ name: 'PM-COM-1302' })
}

useBottomTabSetup({
  value: 'PM-COM-1301',
  label: '현장조치',
  path: '/views/com/PM-COM-1301',
  componentName: 'PmCom1301',
  closable: true,
})
</script>
