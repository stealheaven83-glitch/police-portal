...
import { useTipEdit } from './composable/PM-COM-0715'
...
const navItems = [
  ...
  { label: '게시판' },
  { label: '교육자료 나눔터' },
  { label: '현장대응팁', path: '/views/com/PM-COM-0713' },
]

const { form, writer, writtenAt, fileCount, canSave, addFiles, removeFile } = useTipEdit()
 ...
 * 제목이 없는 경우는 버튼 자체가 비활성(등록 시안 PM-COM-0716 와 같은 규칙)이라 여기까지 오지 않는다.
...
async function onSave() {
  ...
  if (!confirmed) return
  router.push({ name: 'PM-COM-0713' })
}

useBottomTabSetup({
  value: 'PM-COM-0715',
  label: '현장대응팁 수정',
  path: '/views/com/PM-COM-0715',
  componentName: 'PmCom0715',
  closable: true,
})
</script>
