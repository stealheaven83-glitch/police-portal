/* 임시 화면ID: 기획서(범죄예방진단 V 1.2)의 PM-COM-0801~0804 가 기존 PM-COM-0801·0802(통합검색)와 화면ID가 중복돼 PM-COM-0803~0806 으로 임시 작업했다. 이 파일 = 기획서 PM-COM-0803(수정). 화면ID 확정 후 폴더·라우트·워크리스트·board.ts 를 함께 바꿀 것. */
import { computed, reactive } from 'vue'
import type { DepartmentValue } from '@/components/custom/select/DepartmentCascadeSelect.vue'
import { useBoardStore, type BoardFile, type BoardPostForm } from '../../composable/board'

/**
 * 범죄예방진단 수정(PM-COM-0805) 화면 상태.
 * 폼 값은 도메인 스토어(board.ts)의 editForm 을 그대로 쓴다 — 상세(PM-COM-0804)의 '수정' 버튼이
...
export function useCptedEdit() {
  const { current, editForm, loadEditForm } = useBoardStore('cpted')
  ...
  /** 등록 시안(PM-COM-0806)처럼 제목이 비어 있으면 저장 버튼을 비활성으로 둔다 */
