<!-- 임시 화면ID: 기획서(범죄예방진단 V 1.2)의 PM-COM-0801~0804 가 기존 PM-COM-0801·0802(통합검색)와 화면ID가 중복돼 PM-COM-0803~0806 으로 임시 작업했음. 이 파일 = 기획서 PM-COM-0803(수정). 화면ID 확정 후 폴더·라우트·워크리스트·board.ts 를 함께 바꿀 것. -->
<template>
  <PageHeader>
    <template #left>
...
import { useCptedEdit } from './composable/PM-COM-0805'
...
// LNB: 게시판 > 자료실 > 범죄예방진단
useSideMenuSetup({ ...boardMenu, openIndex: 3, activeChild: '범죄예방진단' })
...
const navItems = [
  ...
  { label: '게시판' },
  { label: '자료실' },
  { label: '범죄예방진단', path: '/views/com/PM-COM-0803' },
]

const { form, writer, writtenAt, dept, fileCount, canSave, addFiles, removeFile } = useCptedEdit()
 ...
 * 제목이 없는 경우는 버튼 자체가 비활성(등록 시안 PM-COM-0806 와 같은 규칙)이라 여기까지 오지 않는다.
...
async function onSave() {
  ...
  if (!confirmed) return
  router.push({ name: 'PM-COM-0803' })
}

useBottomTabSetup({
  value: 'PM-COM-0805',
  label: '범죄예방진단 수정',
  path: '/views/com/PM-COM-0805',
  componentName: 'PmCom0805',
  closable: true,
})
</script>
