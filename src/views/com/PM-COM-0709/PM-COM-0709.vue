...
import { useLawList, type LawRow } from './composable/PM-COM-0709'
import {
  boardMenu,
  useBoardStore,
  authorFilterOptions,
...
const { selectPost } = useBoardStore('law')
...
function titleFormatter(cell: any) {
  const row = cell.getData() as LawRow
...
async function onDeleteSelected() {
  ...
  const ids = (gridRef.value?.getSelectedData() ?? []).map((row: LawRow) => row.id)
  removeRows(ids)
...
function onCreate() {
  router.push({ name: 'PM-COM-0712' })
}

/** 제목을 누르면 상세로 이동 */
function onRowClick(_event: unknown, row: { getData: () => { id?: number } }) {
  selectPost(row.getData().id ?? 1)
  router.push({ name: 'PM-COM-0710' })
}

useBottomTabSetup({
  value: 'PM-COM-0709',
  label: '법령 · 지침 · 매뉴얼',
  path: '/views/com/PM-COM-0709',
  componentName: 'PmCom0709',
  closable: true,
})
</script>
