<template>
  ...
  <!-- 시안: 이 게시판은 부서 조회줄이 없다 — 상세조회 토글 없이 항상 펼쳐진다 -->
  <SearchWrapper>
    <template #form>
      <!-- 시안: 1줄 성명·등록일 / 2줄 검색어·공개상태 -->
...
import { useProposalList, type ProposalRow } from './composable/PM-COM-2101'
import {
  boardMenu,
  useBoardStore,
  authorFilterOptions,
...
function titleFormatter(cell: any) {
  const row = cell.getData() as ProposalRow
...
/** 시안: 번호 다음에 공개상태 칸이 온다. 부서·추천수 칸은 없다 */
...
async function onDeleteSelected() {
  ...
  const ids = (gridRef.value?.getSelectedData() ?? []).map((row: ProposalRow) => row.id)
  removeRows(ids)
