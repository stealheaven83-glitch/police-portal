...
import { useBoard2101List } from './composable/PM-COM-2101'
import {
  boardMenu,
  BOARD_SPECS,
  useBoardStore,
...
// LNB: 게시판 > 현장공감 TalkTalk > 정책 제안 및 건의사항 (presets.ts 에 게시판 메뉴가 아직 없어 도메인 composable 의 구성을 쓴다)
...
const { selectPost } = useBoardStore('proposal')
...
function onRegister() {
  router.push({ name: 'PM-COM-2104' })
}

/** 행을 누르면 상세로 간다 */
function onRowClick(_event: unknown, row: { getData: () => { id?: number } }) {
  selectPost(row.getData().id ?? 1)
  router.push({ name: 'PM-COM-2102' })
}

useBottomTabSetup({
  value: 'PM-COM-2101',
  label: '정책 제안 및 건의사항',
  path: '/views/com/PM-COM-2101',
  componentName: 'PmCom2101',
  closable: true,
})
</script>
