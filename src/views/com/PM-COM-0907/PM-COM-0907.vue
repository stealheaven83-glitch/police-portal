<template>
  ...
  <div class="lp-page-scroll">
    <div class="lp-narrow-form">
      <!-- 시안: 작성자 줄 오른쪽 끝에 '공지'·'비밀글' 체크박스가 온다 -->
      <div class="lp-row-between">
        ...
        <span class="lp-radio-inline">
          ...
          <Checkbox v-model="secret" size="lg" label="비밀글" />
        </span>
      </div>
...
import { useProposalEdit } from './composable/PM-COM-0907'
...
const navItems = [
  ...
  { label: '게시판' },
  { label: '현장공감 TalkTalk' },
  { label: '정책 제안 및 건의사항', path: '/views/com/PM-COM-0905' },
]

const { form, writer, writtenAt, secret, fileCount, canSave, addFiles, removeFile } = useProposalEdit()
 ...
 * 제목이 없는 경우는 버튼 자체가 비활성(등록 시안 PM-COM-0908 와 같은 규칙)이라 여기까지 오지 않는다.
...
async function onSave() {
  ...
  if (!confirmed) return
  router.push({ name: 'PM-COM-0905' })
}

useBottomTabSetup({
  value: 'PM-COM-0907',
  label: '정책 제안 및 건의사항 수정',
  path: '/views/com/PM-COM-0907',
  componentName: 'PmCom0907',
  closable: true,
})
</script>
