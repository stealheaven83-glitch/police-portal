<!-- 임시 화면ID: 기획서(범죄예방진단 V 1.2)의 PM-COM-0801~0804 가 기존 PM-COM-0801·0802(통합검색)와 화면ID가 중복돼 PM-COM-0803~0806 으로 임시 작업했음. 이 파일 = 기획서 PM-COM-0801(목록). 화면ID 확정 후 폴더·라우트·워크리스트·board.ts 를 함께 바꿀 것. -->
<template>
  <PageHeader>
    <template #left>
...
import { useCptedList, type CptedRow } from './composable/PM-COM-0803'
import {
  boardMenu,
  useBoardStore,
  authorFilterOptions,
...
function titleFormatter(cell: any) {
  const row = cell.getData() as CptedRow
...
async function onDeleteSelected() {
  ...
  const ids = (gridRef.value?.getSelectedData() ?? []).map((row: CptedRow) => row.id)
  removeRows(ids)
...
useBottomTabSetup({
  value: 'PM-COM-0803',
  label: '범죄예방진단',
  path: '/views/com/PM-COM-0803',
  componentName: 'PmCom0803',
  closable: true,
})
</script>
