/* 임시 화면ID: 기획서(범죄예방진단 V 1.2)의 PM-COM-0801~0804 가 기존 PM-COM-0801·0802(통합검색)와 화면ID가 중복돼 PM-COM-0803~0806 으로 임시 작업했다. 이 파일 = 기획서 PM-COM-0801(목록). 화면ID 확정 후 폴더·라우트·워크리스트·board.ts 를 함께 바꿀 것. */
import { computed, ref, watch } from 'vue'
import type { DepartmentValue } from '@/components/custom/select/DepartmentCascadeSelect.vue'

/**
 * 범죄예방진단 목록(PM-COM-0803) 한 줄.
 *
 * 칸 구성: 부서 O / 공지 고정 O / 추천수 O / 공개상태 X / 카테고리 X.
 * 상세·수정·등록(PM-COM-0804~0704)은 `useBoardStore('cpted')` 의 상태를 쓰고,
 * 목록 rows 는 그 스토어에 없어서 여기서 따로 둔다.
 */
export interface CptedRow {
  id: number
...
function createMockRows(): CptedRow[] {
...
export function useCptedList() {
  ...
  const allRows = ref<CptedRow[]>(createMockRows())
