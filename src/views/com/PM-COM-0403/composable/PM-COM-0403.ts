...
/**
 * Q&A 수정(PM-COM-0403) 화면 상태.
 * 폼 값은 도메인 스토어(qna.ts)의 editForm 을 그대로 쓴다 — 상세(PM-COM-0402)의 '수정' 버튼이
 * loadEditForm() 으로 채우고 이 화면으로 넘어온다(docs/create/multi-route.md §3).
 */
export function useQnaEdit() {
  const { current, editForm, loadEditForm } = useQnaStore()
  ...
  /** 등록/수정 시안(PM-COM-1004)처럼 제목이 비어 있으면 저장 버튼을 비활성으로 둔다 */
