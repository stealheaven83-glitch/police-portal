...
기획서에 동작이 안 적혀 있으면 핸들러가 비어 있어도 **버그가 아니다** — 억지로 만들지 않고
인계 메모(§11)에 적는다. 반대로 기획서에 `[A01] 저장 컨펌창`처럼 적혀 있으면 그건 화면 동작이니
만든다(§7).
...
**⚠ 화면 분할·패널은 module.css 로 직접 만들지 않는다.** `display:grid` 로 2분할을 짜거나
...
### 기준은 **Figma**, 기능은 **PPT** — 역할이 갈린다
Figma 는 PPT 내용을 그대로 반영해 그린 것이다. 그래서:

| | 어디를 보나 |
|---|---|
| **화면이 어떻게 생겼는지** — 레이아웃, 간격, 색, 폰트, 아이콘, 상태별 모양 | **Figma (기준)** |
| **어떻게 동작하는지** — 조회 버튼, 팝업 호출, 필수항목, 컨펌창(`[A01]` 등), 유효성 | **PPT** (Figma 에 없다) |

**스타일은 전부 Figma 기준이다.** PPT 이미지를 보고 눈대중으로 그리지 않는다 — PPT 는 문서에
박힌 캡처라 실제 값이 없다.
...
PPT 이미지를 보고 비슷한 걸 `lucide-vue-next` 에서 골라 쓰지 않는다. Figma 에 아이콘이
...
**rate limit이 잦으니 화면 전체를 훑지 않는다.** 필요한 프레임만 연다.
