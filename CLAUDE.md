...
## 0. 같이 볼 문서 — **절차는 이 파일에만 있다**
**이 문서(CLAUDE.md)가 "이렇게 해라"를 말하는 유일한 곳이다.** 나머지는 "무엇이 있는지"와
"어떻게 동작하는지"만 말한다. 절차가 두 곳에서 나오면 어느 쪽이 최신인지 갈려서 반대로 만들게
된다(§9 가 Figma·PPT 를 두고 말하는 것과 같은 이유). **다른 문서에서 "이렇게 만들어라"를 읽으면
그건 낡은 것이다 — 따르지 말고 알린다.**

CLAUDE.md 는 매 세션 자동으로 전문이 실리고, 아래 문서들은 **필요할 때만 펼쳐 본다.**

| 문서 | 무엇 | 언제 편다 |
|---|---|---|
| `screen-id-map.md` | 화면명·Figma 프레임 이름 → **화면ID 대응표(284행)** | 화면ID를 확정할 때(§0-1 3단계) |
| `component-guide.md` | 케이스별 **"이럴 땐 이 컴포넌트"** 표 | 화면에 뭘 붙일지 정할 때마다 |
| `docs/review.md` | **화면 검토 규칙(공통)** — 무엇을 어떤 순서로 보는가 | **검토 요청을 받았을 때** |
...
- `screen-id-map.md` — **새 화면은 여기서 시작한다.** 화면ID가 정해져야 폴더·라우트·
  `defineOptions` 이름이 정해진다(§2·§4·§5). **번호로 유추하면 틀린다** — Figma `06_장비관리`는
  `PC-LPO-07xx`다.
...
- `docs/review.md` — **검토 요청("검토해줘", "이상한 거 있나 봐줘")을 받으면 반드시 먼저 펼친다.**
  ...
  검토 **결과**는 화면마다 다르므로 `docs/review-{화면ID}.md` 로 따로 남긴다
  (규칙은 공통이라 화면ID가 안 붙고, 결과는 화면별이라 붙는다).
- `menu-tab-guide.md` — **설정값(`openIndex`·`activeChild`)을 정하는 방법은 §5 에 있다.**
  저 문서는 KeepAlive 가 왜 `defineOptions({ name })` 을 요구하는지, 탭을 닫으면 상태가
  어떻게 되는지 같은 **내부 동작**을 설명한다.
- `README.md` — **사람(퍼블리셔) 대상 온보딩 문서라 우리 규칙보다 앞선 시점의 것이다.**
  ⚠ **§3-2 "새 페이지 추가 절차"는 포털 화면**(`/lpo/notebook/write` 같은 URL 계층 페이지)**용이라
  화면ID 화면에는 해당 없다.** 거기 적힌 대로 `router/index.ts` 에 직접 등록하면 `plannedRoutes` 와
  **중복 등록된다** — 라우터는 §4 를 따른다. (README 는 팀 공용 문서라 우리가 고치지 않는다.)
...
⚠ 만든 화면은 `plannedRoutes` 덕에 **브라우저로 바로 열린다**(§4). 단 **LNB 에는 안 뜬다** — 프리셋은
...
| 대형 검색바 + 검색결과(그리드 아님) | `PM-COM-0801`(검색창) + `PM-COM-0802`(결과) | `custom/search/SearchBar.vue` · `SearchKeywordPanel.vue`, 결과는 구역별 목록 + `Pagination simple` |
...
```
⚠ **폴더명·파일명이 화면ID와 글자 하나까지 같아야 한다.** 라우터가 이 경로 규약으로 화면을
...
- **`src/router/index.ts` 도 `plannedRoutes.ts` 도 건드리지 않는다.** 화면 폴더에 `route.ts` 를
  만들지도 않는다(**`route.ts` 방식은 폐기됐다** — 아래 "왜 바뀌었나").
- `router/plannedRoutes.ts` 가 **화면ID 279개를 미리 등록해 뒀다.** 규약대로
  `views/{domain}/{화면ID}/{화면ID}.vue` 를 만들면 그 순간 진짜 화면이 뜬다.
  아직 안 만든 화면은 `views/error/NotReady.vue`("아직 작업하지 않은 화면입니다" + 만들 파일
  경로)가 대신 뜬다. **라우트가 죽지 않는다** — 링크·브레드크럼·LNB 를 미리 걸어도 안전하다.

```ts
// router/plannedRoutes.ts — 이미 이렇게 돼 있다. 읽기만 하고 고치지 않는다.
export const plannedScreens = [
  ['PC-LPO-0601', '관내현황 상세내역'],   // ← [화면ID, 화면 제목]
  …
]
const pageModules = import.meta.glob('../views/*/*/*.vue')   // 실재하는 파일만 잡힌다
// 파일이 있으면 그 화면, 없으면 NotReady 를 붙인다
```
...
| 상황 | 무엇을 한다 |
|---|---|
| `plannedScreens` 에 그 화면ID가 **없다** | 정렬 위치에 **한 줄만** 추가한다(`['PC-XXX-NNNN', '화면명']`) |
| 규약을 **벗어난다** — 화면군(§3 패턴A, `screenGroup` 필요) / `layout` 이 `WorkLayout` 이 아님 / `path` 가 `/views/{domain}/{화면ID}` 가 아님 | `router/index.ts` 배열에 직접 적는다 |

- 규약을 벗어나 `index.ts` 에 직접 적으면 `buildPlannedRoutes` 가 그 화면ID를 **자동으로 건너뛴다**
  (`taken` 필터). 중복 등록 걱정은 안 해도 된다.
- **화면군**(한 컴포넌트가 여러 화면ID)은 반드시 `index.ts` 쪽이다. `screenGroup` meta 가 없으면
  `Layout.vue` 의 `:key` 때문에 탭 이동마다 리마운트된다(§3).
- 새 화면 제목은 `screen-id-map.md` 의 화면명을 그대로 쓴다(`plannedScreens` 가 그 기준이다).

### 왜 바뀌었나 — `route.ts` 는 왜 폐기됐나
882줄짜리 `router/index.ts` 를 화면 추가마다 셋이 동시에 손대서 git 충돌이 상시로 났다. 그래서
화면 폴더마다 `route.ts` 를 두고 자동 수집하게 했었는데, 이 방식엔 **"나중에 걷어오기"** 가
남아 있었다 — 한 사람이 `route.ts` 수십 개를 `index.ts` 로 옮기는 배치 작업이라, **충돌이
없어진 게 아니라 그 시점에 뭉쳐서 터졌다.** 일도 두 번 했다.

`plannedRoutes` 는 그 단계 자체를 없앤다. **목록이 이미 채워져 있어서 새 화면을 만들 때 라우터
파일을 아무도 안 건드린다.** 걷어올 것도 없다.

⚠ **미리 등록해두는 사고는 여전히 원리적으로 안 생긴다.** 라우트를 `index.ts` 에
`component: () => import('../views/com/PC-COM-9999/PC-COM-9999.vue')` 처럼 **리터럴 경로**로
미리 적어두면 Rollup 이 빌드 시점에 정적 분석해서 파일이 없을 때 `Could not resolve ...` 로
**빌드 전체를 실패**시킨다(실측. `vue-tsc` 는 통과해서 넣은 사람은 모르고 배포하는 사람이 막힌다).
`plannedRoutes` 는 경로를 리터럴로 쓰지 않고 `import.meta.glob` 맵에서 꺼내므로 이 함정이 없다.
**규약을 벗어나 `index.ts` 에 직접 적을 때는 이 함정이 그대로 살아 있다 — 파일을 먼저 만든다.**

### 그래도 손대지 않는 것 / 손대야 하는 것

| 화면을 | `plannedRoutes.ts` | `router/index.ts` | `presets.ts`(LNB) |
|---|---|---|---|
| **추가**(규약대로) | ⛔ 안 건드린다(목록에 없을 때만 한 줄) | ⛔ 안 건드린다 | ⛔ 안 건드린다 |
| **추가**(화면군 등 예외) | ⛔ 안 건드린다 | ✅ 직접 적는다 | ⛔ 안 건드린다 |
| **수정** | ⛔ 안 건드린다 | ⛔ 안 건드린다 | ⛔ 안 건드린다 |
| **삭제** | 목록은 그대로 둬도 된다(NotReady 로 바뀔 뿐) | ✅ 직접 적었으면 지운다 | ✅ 해당 `path` 지운다 |
...
⚠ **삭제는 폴더째 지워도 빌드가 안 깨진다** — glob 이 실재 파일만 잡으니 그 화면은 NotReady 로
돌아갈 뿐이다. 다만 **`index.ts` 에 직접 적힌 화면**이면 그 항목을 **같은 커밋에서 함께** 지워야
한다(리터럴 경로라 `Could not resolve ...` 로 빌드가 깨진다). 그 화면이 화면군의 일부였다면
나머지 화면ID 들이 같은 컴포넌트를 가리키고 있는지 먼저 확인한다(§3 패턴A).

### 지금 화면이 왜 안 뜨나 — 확인 순서
1. **NotReady("아직 작업하지 않은 화면입니다")가 뜬다** → 파일 경로가 규약과 다르다.
   `views/{domain}/{화면ID}/{화면ID}.vue` 인지 확인한다(폴더명·파일명이 화면ID와 **정확히** 같아야
   한다. `domain` 은 화면ID 가운데 세 글자 소문자 — `PC-LPO-0601` → `lpo`).
2. **404 가 뜬다** → `plannedScreens` 에 그 화면ID가 없다. 한 줄 추가한다.
3. **엉뚱한 화면이 뜬다** → `index.ts` 에 같은 `name` 이 직접 적혀 있다. 그쪽이 이긴다.

**미등록 화면 찾는 법** — 폴더는 있는데 어디에도 화면ID가 없는 것:
```bash
comm -23 \
  ...
  <(cat \
    ...
    | sort -u)
```
반대로 라우터에만 있고 폴더가 없는 것은 **아직 안 만든 화면**(NotReady)이거나 **화면군**(§3
패턴A)이라 정상이다 — 지우지 않는다.

⚠ **LNB 프리셋은 IA 로 미리 다 채울 수 없다.** IA 의 depth 는 "화면 계층"이고 LNB 는
...
라우터는 화면ID만 알면 되지만(`plannedRoutes`), 프리셋은 그렇지 않은 이유가 이것이다 — 화면
폴더는 자기가 메뉴 트리의 어느 자리에 붙는지 모른다. 그래서 프리셋만은 **배치 때 사람이
판단해서** 넣는다.
...
## 5. LNB/하단탭 — **설정값은 여기서 정한다**
(`menu-tab-guide.md` 는 이게 **왜 그렇게 동작하는지**를 설명한다 — 값은 아래를 따른다, §0)
