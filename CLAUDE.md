...
②를 빠뜨리는 실수가 잦다 — "공통에 없네" 하고 화면 module.css에 만들어 버리면, 다음 화면이
...
**② 없을 때 어디에 만드나:**
- **색·크기·모서리 값** → `police-style.css`에 **토큰(`--Xxx`)으로 추가.** 화면 module.css에
  hex를 박지 않는다.
...
- **화면 전용 `style/PC-XXX-NNNN.module.css`는 예외적으로만** — 그 화면에서만 의미가 있고
  다른 화면에 옮겨 붙일 수 없는 것(그 화면 고유의 배치·특정 그리드 폭 등). 판단이 서지 않으면
  **공통으로 만든다.**
...
1. **파일 구성** — `PC-XXX-NNNN.vue` + `composable/PC-XXX-NNNN.ts`.
   화면 전용 CSS가 없으면 `style/` 폴더를 **만들지 않는다**(0215가 그렇다 — 공통 클래스로 해결).
...
### 기준 파일의 신뢰도
2026-08-31 에 `PC-LPO-0215` 를 §1·§5·§6 기준으로 정비했다(LNB 추가, 죽은 브레드크럼 링크 제거,
불필요한 래퍼 제거, 페이지네이션 예시 추가). 같은 날 LNB 활성 표시 오류를 16개 화면에서 일괄
수정했다. **그래도 기준 파일이 완전무결하다고 가정하지 않는다** — 복사하기 전에 위 8개 항목을
...
views/{domain}/PC-XXX-NNNN/
  ...
  style/PC-XXX-NNNN.module.css  ← 필요할 때만 (§1 CSS 규칙 참고)
  components/                    (그 화면 전용 팝업 등)
```
⚠ `style/` 은 **필수가 아니다.** 공통 클래스·토큰으로 해결되면 만들지 않는다 —
`PC-LPO-0215` 가 그 예다(폴더 자체가 없다). 빈 module.css 를 먼저 만들어 두면
"이미 있으니 여기 쓰자"가 되어 §1의 "공통 우선" 규칙이 무너진다.
...
### Figma — 기준은 PPT, Figma는 "어떻게 그리는지"
**PPT 기획서가 기준이다.** 무엇을 만들지, 항목이 뭔지, 어떤 규칙으로 동작하는지는 전부 PPT가
정한다. **Figma는 그걸 어떻게 그리는지(스타일)를 보여준다.** 둘이 다르면 PPT가 이긴다.

화면이 공통 컴포넌트로 조립되면 간격·폰트·색은 **컴포넌트가 이미 갖고 있다.**
Figma에서 수치를 재서 화면에 옮기지 않는다 — 그러면 화면마다 값이 갈라진다.

**Figma를 여는 건 PPT에 "공통으로 안 되는 새 컨텐츠"가 나왔을 때다:**
```
PPT 에서 새 컨텐츠 발견
  → component-guide.md 에서 공통에 있나 확인
  → 없으면 Figma 에서 그 프레임을 열어 형태 확인
  → 만든다
  → 공통화한다                       ← 이 단계를 빠뜨리지 않는다
```
- **공통화 위치**: 컴포넌트면 `custom/<name>/` + `component-guide.md` 등재(§1),
  스타일이면 `police-style.css` 토큰/클래스(§1 CSS ②). 화면 전용으로 두지 않는다.
...
**rate limit이 잦으니 화면 전체를 훑지 않는다.** 위 흐름에 해당하는 프레임만 연다.
파일키·페이지ID·접근 함정은 `docs/figma-access.md`. 막히면 사용자가 주는 스크린샷으로 작업한다.
...
**이 문서를 보고 화면을 만드는 세션은 "만들기"에만 집중한다.** 값의 타당성·기획서 대조·규칙
