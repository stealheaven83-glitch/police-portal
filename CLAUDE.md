...
## 1-1. 새 화면 만들 때 기준 파일 — "제일 비슷한 거 찾기"를 매번 새로 하지 않는다
기획 이미지만 보고 "이번엔 이 화면이 제일 비슷하네"를 매번 새로 판단하면, 같은 유형인데도
어떤 화면(A1)은 A를 기준으로 만들고 다른 화면(A2)은 A3을 기준으로 만드는 식으로 갈라진다 —
20개 넘는 화면이 이미 서로 다른 스타일로 갈라져 있던 이유가 이거다. 그래서 유형별 기준 파일을
아래처럼 고정해둔다. **이 표에 없는 이유로 다른 화면을 "더 비슷하다"고 판단해서 베이스로
삼지 않는다.**
...
| 탭 + 다중 팝업이 컴포넌트 하나를 공유(패턴A + `useAutoTrigger`) | `PC-LPO-0701` | `screenGroup` 라우팅 필요, §3 패턴A 원조 예시 |
| 2분할 목록+상세, 단일선택 + 팝업 히스토리 | `PM-PUB-0103` | `row.getData()` 방어적 언랩 예시 |
| 목록 + 등록/상세 폼(`TableWrapper`+`InfoTable`) | `PC-LPO-0801` | |
| 목록/상세/등록이 별개 라우트로 나뉨(패턴B 싱글턴) | `PC-PUB-0301`(목록) + `PC-PUB-0302`/`0303`(상세/등록) | 도메인 싱글턴 스토어 공유 |

**절차**:
1. 화면 이미지를 보고 위 표의 유형 중 어디에 해당하는지 먼저 확인한다.
2. 해당하면 **그 기준 파일을 읽고 그대로 참고한다** — `views/` 전체를 다시 뒤져서 "이번엔 뭐가
   더 비슷한가"를 새로 판단하지 않는다.
3. 표에 있는 어떤 유형과도 애매하게 걸치면, 혼자 판단해서 아무거나 고르지 않고 사용자에게
   확인한다(1번 항목 "새로 만들기 전에 없다는 걸 알린다"와 같은 원칙).
4. 표에 없는 새로운 유형이면, 화면을 다 만든 뒤 **이 표에 새 줄로 등록한다** — 그래야 다음에
   같은 유형이 나왔을 때 다른 세션도 같은 파일을 가리키게 된다.

이 표의 기준 파일들은 위 규칙들(§6~§8 포함)을 기준으로 한 번 감사(audit)해서 위반 사항을
고친 상태다 — 예를 들어 `PC-COM-2201`은 `@row-selection-changed`가 데이터가 아니라
RowComponent를 넘기는 걸 그대로 필드 접근하던 버그(§6)와 저장 시 불필요한 confirm(§7)이
있었는데 둘 다 고쳤다. 기준 파일이 바뀌면(리팩터링 등) 이 표도 같이 갱신한다.
...
- **`useSideMenuSetup(presetKey: string)` 은 비동기다.** 같은 tick 에 이어서
  ...
  덮어써버리는 경합이 생긴다 — 실제로 방범협력단체 화면군에서 겪은 버그로, LNB 하위 메뉴가 안
  펼쳐지고 활성 표시도 엉뚱하게 나왔다. 화면마다 다른 activeChild/openIndex 가 필요하면
