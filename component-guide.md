...
>
> 한 행에 라벨-값 쌍을 몇 개 둘지는 `InfoTable`의 `columns`(1~4)로 정한다. **이건 PC 기준이다** —
> 모바일 동작은 아래 표를 본다.
...
> | `value-class` | **값(정보) 영역** |
>
> **모바일(폭 1600 미만)** — 경계는 앱의 유일한 경계다(`style.css` 의 `mo:`, SideMenu·BottomTab·
> TabulatorGrid 와 같은 값). 2026-09-18 에 정리했다. 둘은 **켜지는 방식이 다르다**:
>
> | | prop | 기본 | 하는 일 |
> |---|---|---|---|
> | 표의 칸 배치 | `InfoTable` 의 `:mo-columns` | **1 (켜짐)** | `columns` 가 2~4 여도 모바일에선 한 줄에 한 칸. 모바일에서도 여러 단이어야 하는 표만 `:mo-columns="2"` 로 끈다 |
> | 칸 안 라벨 자리 | `InfoField` 의 `mo` | **`'row'` (꺼짐)** | `mo="col"` 을 준 칸만 라벨이 값 **위로**. 칸마다 값 영역이 달라(입력 하나 / 체크박스 여럿 / 주소 두 줄) 일괄로 올리지 않는다 |
>
> 표 왼쪽 세로선(`border-left`)은 모바일에서 자동으로 지운다(위 가로선은 남는다).
> `InfoField` 의 `layout="column"` 은 **다른 축**이다 — 그건 값 영역 *안쪽*을 세로로 쌓는 것이고
> 폭과 무관하다.
> ```html
> <InfoTable :columns="2" popup>              <!-- 모바일 1단은 자동 -->
>   <InfoField label="부서">{{ dept }}</InfoField>            <!-- 라벨 옆(기본) -->
>   <InfoField label="주번호" mo="col" for="…">…</InfoField>  <!-- 라벨 위 -->
> </InfoTable>
> ```
> ⚠ 예전에 있던 `62.5rem`(2단→1단) · `40rem`(라벨 위로) 두 단계는 **없앴다.** 미디어쿼리 안의
> `rem` 은 루트 폰트(10px)가 아니라 **초기값 16px** 기준이라 실제로는 1000px·640px 이었고,
> 1600~1000 구간은 페이지가 모바일인데 표만 2단으로 남았다. 옛 문서에서 그 숫자를 보면 낡은 것이다.
>
...
| 카드를 세로로 쌓는 목록 | `custom/card-list/CardList.vue` — 목록(간격·0건)만 맡고 한 장은 슬롯. `<Card as="li">` 를 넣거나 직접 짠 `<li>` 를 넣는다. 모바일 전용 화면이 직접 쓰고, 반응형에서는 `TabulatorGrid` 가 좁은 폭일 때 이걸로 카드를 그린다(화면이 할 일 없음) |
