...
- **치수는 유형과 무관하다** — 어느 유형이든 `docs/create/dimensions.md` 를 §4 에서 같이 편다.
...
**펴는 파일**: **`docs/create/dimensions.md`** — 절차 전체가 거기 있다. 아래는 요약이다.

`get_design_context` 의 `gap-[32px]` 는 **그 프레임 한 겹의 값**이라 그대로 믿으면 형제 간격이
전부 같은 값으로 뭉개진다. 구역 단위로 `get_metadata` 를 열어 좌표로 재고(`다음 y − (이전 y + 이전
height)`), **치수표를 먼저 만든 뒤에 코드를 쓴다.** 컴포넌트 `size` 는 어감으로 고르지 말고
`component-guide.md` §8-3 대응표에서 고른다. 기준 파일(§2)의 픽셀값은 검증된 값이 아니다 —
구조·컴포넌트 선택·클래스 이름만 가져오고 치수는 내 시안에서 다시 잰다.

<!-- 상세 절차(토큰 매핑·컴포넌트 충돌 시 보고 형식·대조 방법)는 dimensions.md ①~⑤ -->
...
2. **§4 치수표를 코드와 대조한다 — 여기까지가 AI 세션이 할 수 있는 전부다.**
   표의 각 줄을 CSS 에서 찾아 값이 같은지 본다(시안 px ÷ 10 = rem):
   ```bash
   grep -nE "클래스1|클래스2" public/portal/asset/css/common/police-common.css
   ```
   컴포넌트 `size` 는 `component-guide.md` §8-3 대응표로 본다(시안 높이 → size 이름). 어감으로 고르지
   않는다 — 시안 32 인 배지에 "md 가 무난하겠지" 하고 24 를 넣은 적이 있다.
   틀린 값을 찾으면 고치고, **표와 코드가 같아졌다는 걸 결과 보고에 적는다.**
3. **렌더된 실제 px 는 사람이 확인한다.** 이 저장소에는 헤드리스 브라우저(playwright 등)가 없어
   AI 세션은 화면을 띄워 잴 수 없다. 그러니 **"확인했다" 고 쓸 때는 "코드 대조까지" 라고 밝힌다** —
   안 그러면 §5 를 한 것처럼 보이지만 실제로는 코드만 다시 읽은 것이 된다(그래서 틀린 채로 굳었다).
   사람은 시안과 같은 폭에서 개발자도구로 간격·높이를 본다. 좁아질 때는 안 깨지기만 하면 된다.
   전체 절차는 `docs/create/dimensions.md` ④.
4. **파일이 성립하는지만 본다** — import 누락, 태그 안 닫힘처럼 명백한 것. 전체 `vite build`·`vue-tsc`
   는 굳이 돌리지 않아도 된다(45초씩 걸린다).
5. **값의 타당성·Figma 대조·규칙 준수 감사는 하지 않는다.** 그건 별도 검토 세션(`docs/review.md`)이
   한다. 만든 세션이 스스로 검토하면 자기 논리를 그대로 다시 따라가서 못 잡는다(`docs/rationale.md`).
   2번(치수표 대조)은 감사가 아니라 **내가 옮겨 적은 숫자가 내 표와 같은지** 보는 것이라 여기 해당하지 않는다.
...
3. **확실하지 않은 것** — 값이나 판단이 애매했던 지점을 그대로 적는다. 숨기지 않는다.
   ...
   - 예: "공지 상세는 구분선→이미지 40, Q&A 상세는 32 — 같은 인스턴스인데 시안이 갈림. 둘 다 그대로 옮겼음"
