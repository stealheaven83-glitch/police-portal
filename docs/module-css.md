...
|---|---|---|---|
| `custom/info-table/InfoTable.module.css` | `InfoTable` · `InfoField` | 43개 | `--info-label-w`(기본 `14rem`) |
| `custom/flex-grid/FlexGrid.module.css` | `FlexRow` · `FlexCol` | 2개(`.row` `.col`) | `--flex-col-min-w`(기본 `20rem`) |

### `InfoTable.module.css` — 라벨-값 표
클래스 43개가 역할별로 이렇게 나뉜다. **이름만 보고 짐작하지 말고 파일을 열어 확인한다**:
```bash
...
| 값 표현 | `.info-table-txt` `.info-table-radio` `.info-table-divided` `.flex-wrap` `.disclaimerStrong` |
