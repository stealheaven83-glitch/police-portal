...
> **팝업을 다시 열었을 때 이전 입력이 남는 것은 보지 않는다 — 개발팀 몫이다.**
> 팝업이 닫혀도 그 컴포넌트는 부모 화면에 붙어 있어 `ref` 값이 살아남는데, 무엇을 언제 비울지는
> 저장 성공 여부·재조회 시점과 함께 정해지는 것이라 화면단에서 앞질러 정할 일이 아니다.
> **지적하지도, 고치지도 않는다.** (2026-09-10 결정 — 그전에 넣었던 초기화 `watch` 4개도 걷어냈다)

---

## 어디까지 고치나 — 확실한 것은 검토하면서 고친다

**기본은 목록만 남기는 것**이고, 아래 셋을 **전부** 만족하는 것만 그 자리에서 고친다. 보고 → 지시 →
수정으로 세 번 왕복할 이유가 없는 것들이다.

1. **고치는 방법이 하나뿐이다** — 고르거나 정할 게 없다.
2. **수정이 그 화면 폴더 안에서 끝난다** — `views/{도메인}/{화면ID}/` 밖으로 안 나간다.
3. **도구나 규칙이 판정한다** — 사람 감각이 아니라 아래 중 하나가 근거다.
   - `vue-tsc` 가 내는 오류(구문 오류, `TS6133` 미사용 import·변수)
   - CLAUDE.md §5 함정 패턴(`getData()` 누락, `splice` 제자리 수정, 빈 문자열 sentinel, id/for 중복)
   - CLAUDE.md §3 코드 스타일(블록 순서, import 순서, 세미콜론, prop 케밥케이스)
   - CLAUDE.md §4 피드백 형태(`btnCancel: '확인'`)
   - 규칙이 바뀌어 낡아진 주석("§7 기본은 toast" 등)
   - 참조가 0인 그 폴더 안의 죽은 코드(미사용 import·모듈 CSS 규칙·빈 속성·사실과 다른 주석)

**고치지 않고 목록에만 적는 것** — 하나라도 걸리면 멈춘다.
- **공통 파일을 건드려야 하는 것**: `police-*.css`, `router/index.ts`, `presets.ts`, `components/**`,
  다른 화면의 파일. 3명이 동시 작업이라 pull 충돌이 나고, 내 눈에 안 보이는 사용처가 있다(§1).
- **둘 중 하나를 골라야 하는 것**: 중복 블록 중 어느 쪽이 최신인지, 어떤 값을 넣을지.
- **Figma 대조가 필요한 것**: 제목 크기, 라벨 노출 여부, 문구.
- **동작 정책이 걸린 것**: confirm 을 넣을지 같은 것. **팝업 재오픈 시 초기화는 아예 개발팀 몫이라
  목록에도 적지 않는다**(항목 8 참고).

**멈추는 선** — 한 화면에서 고친 파일이 3개를 넘거나 공통 파일이 끼면 거기서 멈추고 보고한다.
검토 diff 가 커지면 무엇이 검토 결과이고 무엇이 수정인지 구분이 안 된다.

> **겉보기와 원인이 다른 경우를 조심한다.** 실제로 겪은 둘:
> `.lp-perm-menu-grid` 는 "안 쓰는 CSS" 로 보였지만 답은 CSS 삭제가 아니라 템플릿에 클래스를 붙이는
> 것이었고, `.lp-photo-*` 는 "없는 클래스를 쓰는 화면" 으로 보였지만 실은 남의 커밋이 CSS 를 덮어
> 지운 것이었다. **고치기 전에 `git log -S"이름"` 으로 그게 원래 있었는지 한 번 본다.**

## 결과 남기기

발견한 것은 `docs/review-{화면ID}.md` 에 번호를 붙여 적는다(예: `docs/review-PC-LPO-0701.md`).
**고친 것도 지우지 말고 남긴다** — 항목 앞에 `✅ **(날짜 수정됨)**` 을 붙이고, 무엇을 어떻게 바꿨는지
한 줄 덧붙인다. 이게 없으면 다음 사람이 "검토했다는데 왜 이 항목이 없지" 하고 같은 곳을 다시 판다.
