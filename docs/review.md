...
> **현황**: 저장소는 지금 반대가 다수다 — 화면 26개 · 팝업 23개가 `<script setup>` 먼저다.
> **새로 만드는 화면부터** 적용하고 기존은 차차 정리한다(`styles.css`·라우터와 같은 전략).
> ⚠ **기준 파일 `PC-LPO-0215` 도 아직 `<script setup>` 이 먼저다** — 아래 항목 7 과 어긋나므로,
> 0215 를 뒤집거나 항목 7 에서 블록 순서를 빼야 한다.
